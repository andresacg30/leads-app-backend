...
def validate_duplicate(lead: LeadModel, campaign_id: str):
    from app.controllers.lead import get_lead_by_field
    from app.controllers.campaign import get_one_campaign

    campaign: CampaignModel = get_one_campaign(campaign_id)
    existing_lead: LeadModel = get_lead_by_field("phone", lead.phone)
    if existing_lead:
